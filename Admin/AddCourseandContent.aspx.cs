...
public partial class Admin_AddCourseandContent : System.Web.UI.Page
{
	...
	protected void btnSaveCourse_Click(object sender, EventArgs e)
	{
		...
		if (dtCheck.Rows.Count == 0)
		{
			...
			Response.Write("<script>alert('课程名称添加成功！！');window.location=''</script>");
		}
		else if (tbCouName.Text.Trim() == "")
		{
			Response.Write("<script>alert('请填写课程名称！！');window.location=''</script>");
		}
		else
		{
			...
			{
				Response.Write("<script>alert('课程名称已存在！！');window.location=''</script>");
			}
			else
			{
				string val = tbCouName.Text.Trim().ToString();
	...
	protected void btnCourseContent_Click(object sender, EventArgs e)
	{
		...
		if (dtCheck.Rows.Count == 0)
		{
			...
			DBHelper.DBHelper.ExectueNonQuery(sqlInsertOrg);
			Response.Write("<script>alert('目录名称保存成功！！');window.location=''</script>");
		}
		else if (tbAddCourseContent.Text.Trim() == "")
		{
			Response.Write("<script>alert('请填写目录名称！！');window.location=''</script>");
		}
		else
		{
			...
			{
				Response.Write("<script>alert('目录名称已存在！！');window.location=''</script>");
			}
			else
			{
				string val = tbAddCourseContent.Text.Trim().ToString();
